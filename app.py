...
Use formal clinical language. Be specific to the scores. No preamble or closing remarks outside the sections."""
...
def create_pdf_report(path, client_name, bai_total, bai_responses,
    ...
    summary_rows = [
        [Paragraph("<b>Instrument</b>",small_s),Paragraph("<b>Score</b>",small_s),
         Paragraph("<b>Level</b>",small_s),Paragraph("<b>Range Bar</b>",small_s)],
        [Paragraph("<b>Beck Anxiety Inventory (BAI)</b>",ParagraphStyle("bi",fontName="Helvetica-Bold",fontSize=9,textColor=bai_lvl_color)),
         Paragraph(f"<b>{bai_total}/63</b>",ParagraphStyle("bs",fontName="Helvetica-Bold",fontSize=9,textColor=bai_lvl_color,alignment=TA_CENTER)),
         Paragraph(get_bai_level(bai_total),ParagraphStyle("bl",fontName="Helvetica",fontSize=8.5,textColor=bai_lvl_color)),
         Paragraph(f'<font color="{get_bai_color(bai_total)}">{bar(bai_total,63)}</font>',ParagraphStyle("bb",fontName="Courier",fontSize=7))],
        [Paragraph("<b>Penn State Worry Questionnaire (PSWQ)</b>",ParagraphStyle("pi",fontName="Helvetica-Bold",fontSize=9,textColor=pswq_lvl_color)),
         Paragraph(f"<b>{pswq_total}/80</b>",ParagraphStyle("ps",fontName="Helvetica-Bold",fontSize=9,textColor=pswq_lvl_color,alignment=TA_CENTER)),
         Paragraph(get_pswq_level(pswq_total),ParagraphStyle("pl",fontName="Helvetica",fontSize=8.5,textColor=pswq_lvl_color)),
         Paragraph(f'<font color="{get_pswq_color(pswq_total)}">{bar(pswq_total,80)}</font>',ParagraphStyle("pb",fontName="Courier",fontSize=7))],
    ]
    ...
    story.append(sum_table); story.append(Spacer(1, 0.4*cm))
    ...
    SCORE_COLORS = {0:colors.HexColor("#5CB85C"),1:colors.HexColor("#F0AD4E"),
                    2:colors.HexColor("#E07B39"),3:colors.HexColor("#D9534F")}

    bai_rows = [[Paragraph("<b>#</b>",small_s),Paragraph("<b>Symptom</b>",small_s),
                 Paragraph("<b>Score</b>",small_s),Paragraph("<b>Severity</b>",small_s)]]
    for q in BAI_QUESTIONS:
        sc = bai_responses[q["id"]]; sc_col = SCORE_COLORS[sc]
        bai_rows.append([
            Paragraph(str(q["id"]),ParagraphStyle("in",fontName="Helvetica",fontSize=8.5,textColor=WARM,alignment=TA_CENTER)),
            Paragraph(q["text"],body_s),
            Paragraph(f"<b>{sc}</b>",ParagraphStyle("is",fontName="Helvetica-Bold",fontSize=9,textColor=sc_col,alignment=TA_CENTER)),
            Paragraph(SCORE_LABELS[sc],ParagraphStyle("il",fontName="Helvetica",fontSize=8.5,textColor=sc_col)),
        ])
    ...
    bai_styles = [
        ...
        ("LEFTPADDING",(0,0),(-1,-1),6),("ALIGN",(0,0),(0,-1),"CENTER"),("ALIGN",(2,0),(2,-1),"CENTER"),
    ]
    for i in range(1, len(bai_rows)):
        if i % 2 == 0: bai_styles.append(("BACKGROUND",(0,i),(-1,i),LIGHT))
    bai_table.setStyle(TableStyle(bai_styles))
    story.append(bai_table); story.append(Spacer(1, 0.5*cm))
    ...
    pswq_rows = [[Paragraph("<b>#</b>",small_s),Paragraph("<b>Statement</b>",small_s),
                  Paragraph("<b>Raw</b>",small_s),Paragraph("<b>Scored</b>",small_s)]]
    for q in PSWQ_QUESTIONS:
        raw = pswq_responses[q["id"]]; scored = (6-raw) if q["reverse"] else raw
        sc_col = pswq_score_color(scored)
        rev_tag = " <i>[R]</i>" if q["reverse"] else ""
        pswq_rows.append([
            Paragraph(str(q["id"]),ParagraphStyle("pn",fontName="Helvetica",fontSize=8.5,textColor=WARM,alignment=TA_CENTER)),
            Paragraph(q["text"]+rev_tag,ParagraphStyle("pt",fontName="Helvetica",fontSize=9,textColor=DARK,leading=13)),
            Paragraph(str(raw),ParagraphStyle("pr",fontName="Helvetica",fontSize=9,textColor=WARM,alignment=TA_CENTER)),
            Paragraph(f"<b>{scored}</b>",ParagraphStyle("ps2",fontName="Helvetica-Bold",fontSize=9,textColor=sc_col,alignment=TA_CENTER)),
        ])
    ...
    pswq_styles = [
        ...
        ("BOX",(0,0),(-1,-1),0.5,BORDER),("INNERGRID",(0,0),(-1,-1),0.3,BORDER),
        ("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),
        ("LEFTPADDING",(0,0),(-1,-1),6),("ALIGN",(0,0),(0,-1),"CENTER"),("ALIGN",(2,0),(3,-1),"CENTER"),
    ]
    for i in range(1, len(pswq_rows)):
        if i % 2 == 0: pswq_styles.append(("BACKGROUND",(0,i),(-1,i),LIGHT))
    pswq_table.setStyle(TableStyle(pswq_styles))
    story.append(pswq_table)
    story.append(Spacer(1, 0.2*cm))
    story.append(Paragraph("<i>[R] = reverse scored item. Scored value reflects the transformed score used in the total.</i>",
    ...
    for line in report_text.split("\n"):
        line = line.strip()
        if not line: story.append(Spacer(1, 0.18*cm))
        elif line.isupper() or (line.endswith(":") and len(line) < 70) or (
            line[:3] in ("A1.","A2.","A3.","B1.","B2.","B3.","C1.","C2.","C3.")
...
def send_report_email(pdf_path, client_name, bai_total, pswq_total, filename):
    ...
    body_html = f"""<html><body style="font-family:Georgia,serif;color:#1C1917;background:#F7F4F0;padding:24px;">
      <div style="max-width:580px;margin:0 auto;background:white;border:1px solid #DDD5C8;border-radius:4px;padding:32px;">
        ...
        <p style="color:#6B5B45;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;margin-top:0;">BAI · PSWQ — New Submission</p>
        ...
        <table style="width:100%;font-size:13px;border-collapse:collapse;">
          <tr><td style="padding:6px 0;color:#6B5B45;width:50%;">BAI Total Score</td>
              <td><strong style="color:{bai_color};">{bai_total}/63 — {get_bai_level(bai_total)}</strong></td></tr>
          <tr><td style="padding:6px 0;color:#6B5B45;">PSWQ Total Score</td>
              <td><strong style="color:{pswq_color};">{pswq_total}/80 — {get_pswq_level(pswq_total)}</strong></td></tr>
        </table>
        <hr style="border:none;border-top:1px solid #DDD5C8;margin:18px 0;">
        <p style="font-size:13px;line-height:1.6;">The full clinical report is attached as a PDF.</p>
        <p style="font-size:11px;color:#6B5B45;margin-top:20px;font-style:italic;">Confidential — intended only for the treating clinician.</p>
      </div></body></html>"""
...
:root{--bg:#F7F4F0;--white:#FFFFFF;--deep:#1C1917;--warm:#6B5B45;--accent:#8B6F47;--border:#DDD5C8;--selected:#2D2926;}
...
[data-testid="stAppViewContainer"],.stApp{background-color:#F7F4F0!important;color:#1C1917!important;}
html,body,[class*="css"]{font-family:'DM Sans',sans-serif;background-color:var(--bg);color:var(--deep);}
.stApp{background-color:var(--bg);}
.page-header{text-align:center;padding:2.5rem 0 2rem;border-bottom:1px solid var(--border);margin-bottom:2rem;}
.page-header h1{font-family:'Playfair Display',serif;font-size:2.4rem;font-weight:400;letter-spacing:.02em;margin-bottom:.3rem;color:var(--deep);}
.page-header p{color:var(--warm);font-size:.82rem;letter-spacing:.1em;text-transform:uppercase;font-weight:400;}
.section-divider{text-align:center;padding:1.5rem 0 1rem;border-bottom:1px solid var(--border);margin:2rem 0 1.5rem;}
.section-divider h2{font-family:'Playfair Display',serif;font-size:1.6rem;font-weight:400;color:var(--deep);margin-bottom:.2rem;}
.section-divider p{color:var(--warm);font-size:.78rem;letter-spacing:.1em;text-transform:uppercase;}
.question-card{background:var(--white);border:1px solid var(--border);border-radius:6px;padding:1.5rem 1.8rem .5rem;margin-bottom:1rem;transition:border-color .2s;}
.question-card:hover{border-color:var(--accent);}
.q-number{font-size:.7rem;letter-spacing:.12em;text-transform:uppercase;color:var(--accent);margin-bottom:.3rem;font-weight:500;}
.q-text{font-family:'Playfair Display',serif;font-size:1.08rem;color:var(--deep);margin-bottom:1rem;line-height:1.5;}
div[data-testid="stRadio"]>label{display:none;}
div[data-testid="stRadio"]>div{gap:.4rem!important;flex-direction:row!important;flex-wrap:wrap!important;}
div[data-testid="stRadio"]>div>label{background:var(--bg)!important;border:1px solid var(--border)!important;border-radius:3px!important;padding:.5rem .9rem!important;cursor:pointer!important;font-size:.82rem!important;color:var(--deep)!important;font-family:'DM Sans',sans-serif!important;transition:all .15s ease!important;min-width:180px!important;}
div[data-testid="stRadio"]>div>label:hover{border-color:var(--accent)!important;background:#F0EBE3!important;}
.progress-wrap{background:var(--border);border-radius:2px;height:3px;margin:1.5rem 0 .5rem;}
.progress-fill{height:3px;border-radius:2px;background:linear-gradient(90deg,var(--warm),var(--accent));}
.stButton>button{background:var(--selected)!important;color:var(--bg)!important;border:none!important;padding:.8rem 2.8rem!important;font-family:'DM Sans',sans-serif!important;font-size:.83rem!important;font-weight:500!important;letter-spacing:.1em!important;text-transform:uppercase!important;border-radius:2px!important;transition:background .2s ease!important;}
.stButton>button:hover{background:var(--warm)!important;}
.thank-you{text-align:center;padding:5rem 2rem;}
.thank-you h2{font-family:'Playfair Display',serif;font-size:2.2rem;font-weight:400;margin-bottom:1rem;}
.thank-you p{color:var(--warm);font-size:.95rem;max-width:380px;margin:0 auto;line-height:1.8;}
div[data-testid="stTextInput"] input{background:white!important;border:1px solid var(--border)!important;border-radius:3px!important;font-family:'DM Sans',sans-serif!important;color:var(--deep)!important;}
</style>""", unsafe_allow_html=True)

page = st.query_params.get("page", "client")

if page == "admin":
    st.markdown("""<div class="page-header"><p>Therapist Portal</p><h1>Assessment Reports</h1></div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.admin_auth:
        ...
        if not files: st.info("No reports submitted yet.")
        else:
    ...
    if st.session_state.submitted:
        st.markdown("""<div class="thank-you"><h2>Thank You</h2>
        ...
        <p style="font-size:.88rem;color:#6B5B45;text-align:center;margin-bottom:1.5rem;line-height:1.8;">
        ...
        for q in BAI_QUESTIONS:
            ...
            choice = st.radio(label=f"bai_{qid}", options=list(BAI_SCALE.values()),
                              index=None, key=f"bai_{qid}", label_visibility="collapsed", horizontal=True)
            if choice is None: bai_all_answered = False
            else: bai_responses[qid] = next(k for k, v in BAI_SCALE.items() if v == choice)
        ...
        for q in PSWQ_QUESTIONS:
            ...
            choice = st.radio(label=f"pswq_{qid}", options=list(PSWQ_SCALE.values()),
                              index=None, key=f"pswq_{qid}", label_visibility="collapsed", horizontal=True)
            if choice is None: pswq_all_answered = False
            else: pswq_responses[qid] = next(k for k, v in PSWQ_SCALE.items() if v == choice)
        ...
        st.markdown(f"""<div style="text-align:center;font-size:.78rem;color:#6B5B45;
        ...
        if not all_answered and answered > 0:
            st.markdown("""<div style="background:#FFF8F0;border-left:3px solid #E07B39;
                        ...
                        font-size:.88rem;color:#7A3D1A;margin:1rem 0;">
        ...
        if submit and all_answered:
            with st.spinner("Submitting your responses..."):
                ...
                report_text = generate_report(client_name or "Anonymous",
